...
impl Gallery {
    ...
    pub fn build(&mut self, ctx: &mut Context) {
        ...
        // Demo Window
        if window::begin_window(ctx, "Demo Window", demo_rect, Options::empty())
            != Response::empty()
        {
            self.build_demo(ctx);
            window::end_window(ctx);
        }

        // Log Window
        if window::begin_window(ctx, "Log Window", log_rect, Options::empty())
            != Response::empty()
        {
            self.build_log(ctx);
            window::end_window(ctx);
        ...
        // Style Editor
        if window::begin_window(ctx, "Style Editor", style_rect, Options::empty())
            != Response::empty()
        {
    ...
    fn section_info(&self, ctx: &mut Context) {
        if window::begin_tree(ctx, "Window Info", Options::empty()) == Response::empty() {
            return;
        }
    ...
    fn section_buttons(&mut self, ctx: &mut Context) {
        if window::begin_tree(ctx, "Test Buttons", Options::IS_EXPANDED) == Response::empty() {
            return;
        }
        ...
        if controls::button(ctx, "Popup", None, Options::empty()) != Response::empty() {
            window::open_popup(ctx, "Test Popup");
        }

        if window::begin_popup(ctx, "Test Popup") != Response::empty() {
            controls::button(ctx, "Hello", None, Options::empty());
            controls::button(ctx, "World", None, Options::empty());
            window::end_popup(ctx);
        }
        window::end_tree(ctx);
    }
    ...
    fn section_tree(&mut self, ctx: &mut Context) {
        if window::begin_tree(ctx, "Tree and Text", Options::IS_EXPANDED) == Response::empty() {
            return;
        }
        ctx.layout().row(2, &[140, -1], 0);

        // Left column – nested trees
        window::begin_panel(ctx, "tree_col", Options::NO_FRAME);
        if window::begin_tree(ctx, "Test 1", Options::empty()) != Response::empty() {
            if window::begin_tree(ctx, "Test 1a", Options::empty()) != Response::empty() {
                controls::label(ctx, "Hello");
                controls::label(ctx, "world");
                window::end_tree(ctx);
            }
            if window::begin_tree(ctx, "Test 1b", Options::empty()) != Response::empty() {
                if controls::button(ctx, "Button 1", None, Options::empty()) != Response::empty() {
                    self.log_text("Pressed button 1");
                }
                if controls::button(ctx, "Button 2", None, Options::empty()) != Response::empty() {
        ...
        if window::begin_tree(ctx, "Test 2", Options::empty()) != Response::empty() {
        ...
        if window::begin_tree(ctx, "Test 3", Options::empty()) != Response::empty() {
        ...
        window::end_panel(ctx);

        // Right column – lorem ipsum + textarea
        window::begin_panel(ctx, "text_col", Options::NO_FRAME);
    ...
    fn section_canvas(&mut self, ctx: &mut Context) {
        if window::begin_tree(ctx, "Background Color", Options::IS_EXPANDED) == Response::empty() {
            return;
        }
        ctx.layout().row(2, &[-78, -1], 74);

        // Sliders column
        window::begin_panel(ctx, "canvas_sliders", Options::NO_FRAME);
        ctx.layout().row(2, &[46, -1], 0);
        controls::label(ctx, "Red:");
        controls::slider(ctx, &mut self.canvas[0], 0.0, 255.0, 1.0, "%.0f", Options::empty());
        controls::label(ctx, "Green:");
        controls::slider(ctx, &mut self.canvas[1], 0.0, 255.0, 1.0, "%.0f", Options::empty());
        controls::label(ctx, "Blue:");
        controls::slider(ctx, &mut self.canvas[2], 0.0, 255.0, 1.0, "%.0f", Options::empty());
        window::end_panel(ctx);
    ...
    fn section_picture(&self, ctx: &mut Context) {
        if window::begin_tree(ctx, "Image Render", Options::IS_EXPANDED) == Response::empty() {
            return;
        }
        ctx.layout().row(1, &[-1], 0);
        match self.picture {
    ...
    fn build_log(&mut self, ctx: &mut Context) {
        ...
        window::begin_panel(ctx, "Log Output", Options::empty());
